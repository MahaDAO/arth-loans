...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    ...
    function _triggerMAHAIssuance(ICommunityIssuance _communityIssuance) internal {
        uint MAHAIssuance = _communityIssuance.issueMAHA();
       _updateG(MAHAIssuance);
    }

    function _updateG(uint _MAHAIssuance) internal {
        ...
        *
        */
        if (totalLUSD == 0 || _MAHAIssuance == 0) {return;}

        uint MAHAPerUnitStaked;
        MAHAPerUnitStaked =_computeMAHAPerUnitStaked(_MAHAIssuance, totalLUSD);

        uint marginalMAHAGain = MAHAPerUnitStaked.mul(P);
        epochToScaleToG[currentEpoch][currentScale] = epochToScaleToG[currentEpoch][currentScale].add(marginalMAHAGain);

        emit G_Updated(epochToScaleToG[currentEpoch][currentScale], currentEpoch, currentScale);
    }

    function _computeMAHAPerUnitStaked(uint _MAHAIssuance, uint _totalLUSDDeposits) internal returns (uint) {
        /*
        ...
        */
        uint MAHANumerator = _MAHAIssuance.mul(DECIMAL_PRECISION).add(lastMAHAError);

        uint MAHAPerUnitStaked = MAHANumerator.div(_totalLUSDDeposits);
    ...
    * d0 is the last recorded deposit value.
    */
    function getDepositorMAHAGain(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {return 0;}
        ...
        */
        ...
        return kickbackRate.mul(_getMAHAGainFromSnapshots(initialDeposit, snapshots)).div(DECIMAL_PRECISION);
    }
    ...
    */
    function getFrontEndMAHAGain(address _frontEnd) public view override returns (uint) {
        ...
        return frontEndShare.mul(_getMAHAGainFromSnapshots(frontEndStake, snapshots)).div(DECIMAL_PRECISION);
    }

    function _getMAHAGainFromSnapshots(uint initialStake, Snapshots memory snapshots) internal view returns (uint) {
       /*
        ...
        */
        ...
        uint MAHAGain = initialStake.mul(firstPortion.add(secondPortion)).div(P_Snapshot).div(DECIMAL_PRECISION);
    ...
    */
    ...
    function _payOutMAHAGains(ICommunityIssuance _communityIssuance, address _depositor, address _frontEnd) internal {
        // Pay out front end's MAHA gain
        if (_frontEnd != address(0)) {
            uint frontEndMAHAGain = getFrontEndMAHAGain(_frontEnd);
        ...
        uint depositorMAHAGain = getDepositorMAHAGain(_depositor);
