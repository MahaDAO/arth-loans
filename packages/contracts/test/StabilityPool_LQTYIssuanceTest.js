...
contract('StabilityPool - LQTY Rewards', async accounts => {
  ...
  describe("LQTY Rewards", async () => {
    ...
    it("liquidation < 1 minute after a deposit does not change totalLQTYIssued", async () => {
      ...
      assert.isTrue(totalLQTYIssued_1.eq(toBN('0')))
      ...
      assert.isTrue(totalLQTYIssued_2.eq(toBN('0')))
    ...
    it("withdrawFromSP(): reward term G does not update when no LQTY is issued", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(1000, 'ether') , A, A, ZERO_ADDRESS, { from: A })
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: A })
      ...
      assert.isTrue(LQTYIssuedBefore.eq(toBN('0')))
      ...
      assert.isTrue(LQTYIssuedAfter.eq(toBN('0')))
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether') , whale, whale, ZERO_ADDRESS, { from: whale })

      await weth.deposit({ from: A, value: dec(100, 'ether') });
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), { from: A });
      await borrowerOperations.openTrove(th._100pct, dec(1, 22), dec(100, 'ether') , A, A, ZERO_ADDRESS, { from: A })

      await weth.deposit({ from: B, value: dec(100, 'ether') });
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), { from: B });
      await borrowerOperations.openTrove(th._100pct, dec(1, 22), dec(100, 'ether'), B, B, ZERO_ADDRESS, { from: B })

      await weth.deposit({ from: C, value: dec(100, 'ether') });
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), { from: C });
      await borrowerOperations.openTrove(th._100pct, dec(1, 22), dec(100, 'ether'), C, C, ZERO_ADDRESS, { from: C })
      
      await weth.deposit({ from: D, value: dec(100, 'ether') });
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), { from: D });
      await borrowerOperations.openTrove(th._100pct, dec(1, 22), dec(100, 'ether') , D, D, ZERO_ADDRESS, { from: D })
      ...
      const expectedLQTYGain_1yr = toBN('0')  //  communityLQTYSupply.div(toBN('2')).div(toBN('3'))
      ...
      assert.isAtMost(getDifference(A_LQTYGain_1yr, expectedLQTYGain_1yr), 0)
      assert.isAtMost(getDifference(B_LQTYGain_1yr, expectedLQTYGain_1yr), 0)
      assert.isAtMost(getDifference(C_LQTYGain_1yr, expectedLQTYGain_1yr), 0)
      ...
      const expectedLQTYGain_2yr = toBN('0')  //  communityLQTYSupply.mul(toBN('3')).div(toBN('4')).div(toBN('3'))
      ...
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(A)), expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(B)), expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(C)), expectedLQTYGain_2yr), 0)
    })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await weth.deposit({ from: whale, value: dec(10000, 'ether') });
      await weth.approve(borrowerOperations.address, dec(10000, 'ether'), { from: whale });
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), dec(10000, 'ether') , whale, whale, ZERO_ADDRESS, { from: whale })

      await weth.deposit({ from: A, value: dec(200, 'ether')  });
      await weth.approve(borrowerOperations.address, dec(200, 'ether') , { from: A });
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(200, 'ether'), A, A, ZERO_ADDRESS, { from: A})

      await weth.deposit({ from: B, value:  dec(300, 'ether') });
      await weth.approve(borrowerOperations.address,  dec(300, 'ether'), { from: B });
      await borrowerOperations.openTrove(th._100pct, dec(20000, 18),  dec(300, 'ether'), B, B, ZERO_ADDRESS, { from: B })
      
      await weth.deposit({ from: C, value: dec(400, 'ether') });
      await weth.approve(borrowerOperations.address, dec(400, 'ether'), { from: C });
      await borrowerOperations.openTrove(th._100pct, dec(30000, 18), dec(400, 'ether'), C, C, ZERO_ADDRESS, { from: C })
      
      await weth.deposit({ from: D, value: dec(100, 'ether') });
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), { from: D });
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(100, 'ether'), D, D, ZERO_ADDRESS, { from: D })
      ...
      const A_expectedLQTYGain_1yr = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued after 1 year
        // .div(toBN('6'))  // A gets 1/6 of the issuance

      const B_expectedLQTYGain_1yr = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued after 1 year
        // .div(toBN('3'))  // B gets 2/6 = 1/3 of the issuance

      const C_expectedLQTYGain_1yr = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued after 1 year
      ...
      assert.isAtMost(getDifference(A_LQTYGain_1yr, A_expectedLQTYGain_1yr), 0)
      assert.isAtMost(getDifference(B_LQTYGain_1yr, B_expectedLQTYGain_1yr), 0)
      assert.isAtMost(getDifference(C_LQTYGain_1yr, C_expectedLQTYGain_1yr), 0)
      ...
      const A_expectedLQTYGain_2yr = toBN('0') // communityLQTYSupply
        // .mul(toBN('3')).div(toBN('4')) // 75% of total issued after 1 year
        // .div(toBN('6'))  // A gets 1/6 of the issuance

      const B_expectedLQTYGain_2yr = toBN('0') // communityLQTYSupply
        // .mul(toBN('3')).div(toBN('4')) // 75% of total issued after 1 year
        // .div(toBN('3'))  // B gets 2/6 = 1/3 of the issuance

      const C_expectedLQTYGain_2yr = toBN('0') // communityLQTYSupply
        // .mul(toBN('3')).div(toBN('4')) // 75% of total issued after 1 year
        // .div(toBN('2'))  // C gets 3/6 = 1/2 of the issuance
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(getDifference(A_LQTYGain_2yr, A_expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference(B_LQTYGain_2yr, B_expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference(C_LQTYGain_2yr, C_expectedLQTYGain_2yr), 0)
      ...
      // Check LQTY balances increase by correct amount
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(A)), A_expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(B)), B_expectedLQTYGain_2yr), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(C)), C_expectedLQTYGain_2yr), 0)
    })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, dec(40000, 18), dec(500, 'ether'), D, D, ZERO_ADDRESS, { from: D })

      await weth.deposit({from: E, value: dec(600, 'ether') })
      await weth.approve(borrowerOperations.address, dec(600, 'ether') , {from: E})
      await borrowerOperations.openTrove(th._100pct, dec(40000, 18), dec(600, 'ether'), E, E, ZERO_ADDRESS, { from: E })
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(30000, 18)), dec(300, 'ether'), defaulter_1, defaulter_1, ZERO_ADDRESS, { from: defaulter_1 })
      ...
      // Expected gains for each depositor after 1 year (50% total issued)
      const A_expectedLQTYGain_Y1 = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued in Y1
        // .div(toBN('6'))  // A got 1/6 of the issuance

      const B_expectedLQTYGain_Y1 = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued in Y1
        // .div(toBN('3'))  // B gets 2/6 = 1/3 of the issuance

      const C_expectedLQTYGain_Y1 = toBN('0') // communityLQTYSupply
        // .div(toBN('2')) // 50% of total issued in Y1
        // .div(toBN('2'))  // C gets 3/6 = 1/2 of the issuance
      ...
      // Check gains are correct, error tolerance = 1e-6 of a toke
      assert.isAtMost(getDifference(A_LQTYGain_Y1, A_expectedLQTYGain_Y1), 0)
      assert.isAtMost(getDifference(B_LQTYGain_Y1, B_expectedLQTYGain_Y1), 0)
      assert.isAtMost(getDifference(C_LQTYGain_Y1, C_expectedLQTYGain_Y1), 0)
      ...
      const A_expectedLQTYGain_Y2 = toBN('0') // communityLQTYSupply
        // .div(toBN('4')) // 25% of total issued in Y2
        // .div(toBN('14'))  // A got 50/700 = 1/14 of the issuance

      const B_expectedLQTYGain_Y2 = toBN('0') // communityLQTYSupply
        // .div(toBN('4')) // 25% of total issued in Y2
        // .div(toBN('7'))  // B got 100/700 = 1/7 of the issuance

      const C_expectedLQTYGain_Y2 = toBN('0') // communityLQTYSupply
        // .div(toBN('4')) // 25% of total issued in Y2
        // .mul(toBN('3')).div(toBN('14'))  // C gets 150/700 = 3/14 of the issuance

      const D_expectedLQTYGain_Y2 = toBN('0') // communityLQTYSupply
        // .div(toBN('4')) // 25% of total issued in Y2
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.isAtMost(getDifference(A_LQTYGain_AfterY2, A_expectedTotalGain), 0)
      assert.isAtMost(getDifference(B_LQTYGain_AfterY2, B_expectedTotalGain), 0)
      assert.isAtMost(getDifference(C_LQTYGain_AfterY2, C_expectedTotalGain), 0)
      assert.isAtMost(getDifference(D_LQTYGain_AfterY2, D_expectedTotalGain), 0)
      ...
      // Check LQTY balances increase by correct amount
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(A)), A_expectedTotalGain), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(B)), B_expectedTotalGain), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(C)), C_expectedTotalGain), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(D)), D_expectedTotalGain), 0)
    })
    ...
    it('withdrawFromSP(): Depositor withdraws correct LQTY gain after serial pool-emptying liquidations. No front-ends.', async () => {
      ...
      await weth.deposit({from: defaulter_1, value: dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether'), {from: defaulter_1})
      ...
      await weth.deposit({from: defaulter_3, value: dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether'), {from: defaulter_3})
      ...
      await weth.deposit({from: defaulter_4, value: dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether'), {from: defaulter_4})
      ...
      for (account of depositors_1) {
        ...
        await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(200, 'ether'), account, account, ZERO_ADDRESS, { from: account  })
      ...
      const expectedLQTYGain_M1 = toBN('0') // issuance_M1.div(th.toBN('2'))
      const expectedLQTYGain_M2 = toBN('0') // issuance_M2.div(th.toBN('2'))
      const expectedLQTYGain_M3 = toBN('0') // issuance_M3.div(th.toBN('2'))
      const expectedLQTYGain_M4 = toBN('0') // issuance_M4.div(th.toBN('2'))
    ...
    it('LQTY issuance for a given period is not obtainable if the SP was empty during the period', async () => {
      ...
      await weth.deposit({from: A, value: dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether'), {from: A})
      await borrowerOperations.openTrove(th._100pct, dec(16000, 18),  dec(200, 'ether'), A, A, ZERO_ADDRESS, { from: A })

      await weth.deposit({from: B, value: dec(100, 'ether')})
      await weth.approve(borrowerOperations.address, dec(100, 'ether'), {from: B})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18),  dec(100, 'ether'), B, B, ZERO_ADDRESS, { from: B })

      await weth.deposit({from: C, value: dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether'), {from: C})
      await borrowerOperations.openTrove(th._100pct, dec(16000, 18),  dec(200, 'ether'), C, C, ZERO_ADDRESS, { from: C })
      ...
      assert.isTrue(totalLQTYissuance_1.eq(totalLQTYissuance_0))
      assert.isTrue(totalLQTYissuance_1.eq(toBN('0')))
      ...
      assert.isTrue(totalLQTYissuance_2.eq(totalLQTYissuance_1))
      assert.isTrue(totalLQTYissuance_2.eq(toBN('0')))
      ...
      assert.isTrue(totalLQTYissuance_3.eq(totalLQTYissuance_2))
      assert.isTrue(totalLQTYissuance_3.eq(toBN('0')))
      ...
      assert.isTrue(totalLQTYissuance_4.eq(totalLQTYissuance_3))
      assert.isTrue(totalLQTYissuance_4.eq(toBN('0')))
      ...
      assert.equal(A_LQTYGain, '0')

      // Check C earns gains from M4 only
      assert.equal(C_LQTYGain, '0')
      ...
      assert.equal(CIBalanceDifference, '0')
    })
    ...
    it("withdrawFromSP(): Several deposits of 100 LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), dec(100, 'ether'), whale, whale, ZERO_ADDRESS, { from: whale })
      ...
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: A, })
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: B, })
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: C,  })
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: D,})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: E, })
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(10000, 'ether'), ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, { from: F, })
      ...
      for (const defaulter of fiveDefaulters) {
        ...
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount('9999900000000000000000'), dec(100, 'ether'), defaulter, defaulter, ZERO_ADDRESS, { from: defaulter})
      }
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), dec(100, 'ether'), defaulter_6, defaulter_6, ZERO_ADDRESS, { from: defaulter_6})
     ...
     to the liquidation that mostly depleted it.  Error tolerance = 1e-3 tokens. */
      ...
      assert.equal(LQTYGain_A, '0')
      assert.equal(LQTYGain_B, '0')
      assert.equal(LQTYGain_C, '0')
      assert.equal(LQTYGain_D, '0')

      assert.equal(LQTYGain_E, '0')
      assert.equal(LQTYGain_F, '0')
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct LQTY gain. No liquidations. Front ends and kickback rates.", async () => {
      ...
      await weth.deposit({from: A, value:  dec(100, 'ether')})
      await weth.approve(borrowerOperations.address,  dec(100, 'ether'), {from: A})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18),  dec(100, 'ether'), A, A, ZERO_ADDRESS, { from: A })
      ...
      await weth.deposit({from: C, value: dec(100, 'ether') })
      await weth.approve(borrowerOperations.address, dec(100, 'ether') , {from: C})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(100, 'ether') , C, C, ZERO_ADDRESS, { from: C })

      await weth.deposit({from: D, value: dec(10000, 'ether')})
      await weth.approve(borrowerOperations.address, dec(10000, 'ether'), {from: D})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(100, 'ether'), D, D, ZERO_ADDRESS, { from: D  })

      await weth.deposit({from: E, value: dec(100, 'ether') })
      await weth.approve(borrowerOperations.address,dec(100, 'ether') , {from: E})
      await borrowerOperations.openTrove(th._100pct, dec(10000, 18), dec(100, 'ether') , E, E, ZERO_ADDRESS, { from: E })
      ...
      // Check gains are correct, error tolerance = 1e-6 of a token
      assert.equal(A_LQTYGain_Y1, '0')
      assert.equal(B_LQTYGain_Y1, '0')
      assert.equal(C_LQTYGain_Y1, '0')
      assert.equal(D_LQTYGain_Y1, '0')
      assert.equal(F1_LQTYGain_Y1, '0')
      assert.equal(F2_LQTYGain_Y1, '0')
      ...
      // Check LQTY balances increase by correct amount
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(A)), toBN('0')), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(B)), toBN('0')), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(C)), toBN('0')), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(D)), toBN('0')), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(frontEnd_1)), toBN('0')), 0)
      assert.isAtMost(getDifference((await lqtyToken.balanceOf(frontEnd_2)), toBN('0')), 0)
    })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. Front ends and kickback rates", async () => {
      ...
      await weth.approve(borrowerOperations.address,  dec(800, 'ether') , {from: B})
      await borrowerOperations.openTrove(th._100pct, dec(60000, 18), dec(800, 'ether'), B, B, ZERO_ADDRESS, { from: B })
      ...
      await borrowerOperations.openTrove(th._100pct, dec(30000, 18), dec(400, 'ether'), E, E, ZERO_ADDRESS, { from: E })
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(50000, 18)), dec(500, 'ether') , defaulter_1, defaulter_1, ZERO_ADDRESS, { from: defaulter_1 })
      ...
      assert.isAtMost(getDifference(A_LQTYGain_M1, toBN('0')), 0)
      assert.isAtMost(getDifference(B_LQTYGain_M1, toBN('0')), 0)
      assert.isAtMost(getDifference(C_LQTYGain_M1, toBN('0')), 0)
      assert.isAtMost(getDifference(D_LQTYGain_M1, toBN('0')), 0)
      assert.isAtMost(getDifference(F1_LQTYGain_M1, toBN('0')), 0)
      assert.isAtMost(getDifference(F2_LQTYGain_M1, toBN('0')), 0)
      ...
      assert.isAtMost(getDifference(A_LQTYGain_After_M2, toBN('0')), 0)
      assert.isAtMost(getDifference(B_LQTYGain_After_M2, toBN('0')), 0)
      assert.isAtMost(getDifference(C_LQTYGain_After_M2, toBN('0')), 0)
      assert.isAtMost(getDifference(D_LQTYGain_After_M2, toBN('0')), 0)
      assert.isAtMost(getDifference(E_LQTYGain_After_M2, toBN('0')), 0)
      ...
      assert.isAtMost(getDifference(F1_LQTYBalance_After_M2, toBN('0')), 0)

      // Check F1's LQTY gain in system after M2: Just their gain due to M2
      assert.isAtMost(getDifference(F1_LQTYGain_After_M2, toBN('0')), 1e15)

      // Check F2 LQTY gain in system after M2: the sum of their gains from M1 + M2
      assert.isAtMost(getDifference(F2_LQTYGain_After_M2, toBN('0')), 1e15)
      ...
      assert.isAtMost(getDifference(toBN('0'), A_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), B_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), C_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), D_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), E_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), F1_FinalLQTYBalance), 0)
      assert.isAtMost(getDifference(toBN('0'), F2_FinalLQTYBalance), 0)
    })
    ...
    it("withdrawFromSP(): Several deposits of 10k LUSD span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      for (const defaulter of _4_Defaulters) {
        ...
        await weth.deposit({from: defaulter, value:  dec(100, 'ether') })
        await weth.approve(borrowerOperations.address,  dec(100, 'ether'), {from: defaulter})
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 17)),  dec(100, 'ether'), defaulter, defaulter, ZERO_ADDRESS, { from: defaulter})
      }
      ...
      await weth.deposit({from: A, value: dec(200, 'ether') })
      await weth.approve(borrowerOperations.address,  dec(200, 'ether'), {from: A})
      await borrowerOperations.openTrove(th._100pct, dec(5000, 18), dec(200, 'ether'), A, A, ZERO_ADDRESS, { from: A })
      ...
      await borrowerOperations.openTrove(th._100pct, dec(5000, 18), dec(200, 'ether') , B, B, ZERO_ADDRESS, { from: B })
      ...
      await weth.deposit({from: C, value:  dec(200, 'ether')})
      await weth.approve(borrowerOperations.address, dec(200, 'ether') , {from: C})
      await borrowerOperations.openTrove(th._100pct, dec(99999, 17), dec(200, 'ether'), C, C, ZERO_ADDRESS, { from: C })
      ...
      await borrowerOperations.openTrove(th._100pct, dec(99999, 17), dec(200, 'ether') , D, D, ZERO_ADDRESS, { from: D})
      ...
      await borrowerOperations.openTrove(th._100pct, dec(99999, 17),  dec(200, 'ether') , E, E, ZERO_ADDRESS, { from: E })
     ...
     */
      ...
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_A), 0)
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_B), 0)
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_C), 0)
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_D), 0)
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_E), 0)
      assert.isAtMost(getDifference(toBN('0'), LQTYGain_F1), 0)
    })
